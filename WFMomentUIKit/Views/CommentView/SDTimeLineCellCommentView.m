...
- (void)onClickComment:(UITapGestureRecognizer *)sender {
    ...
    for (int i = 0; i < self.commentLabelsArray.count; i++) {
        if ([[self.commentLabelsArray objectAtIndex:i] isEqual:cell]) {
            SDTimeLineCellCommentItemModel *model = self.commentItemsArray[i];
            if (self.didClickCommentLabelBlock) {
                self.didClickCommentLabelBlock(model.comment.commentUid, model.comment.sender, rect, cell);
            }
        }
    }
}
